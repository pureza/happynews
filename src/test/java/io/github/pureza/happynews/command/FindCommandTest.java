...
public class FindCommandTest extends AbstractTest {
    ...
    public void failsWhenNotEnoughParameters() throws IOException {
        ...
        new FindCommand(user, "FIND arg1", server).process();
    ...
    public void failsWhenGivenTooManyParameters() throws IOException {
        User user = mockReader("reader");
        Supplier<String> out = mockInput(user, "");
        new FindCommand(user, "FIND arg1 arg2 arg3", server).process();
    ...
    public void printsMatchingArticles() throws IOException {
        ...
        User user = mockReader("reader");
        Supplier<String> out = mockInput(user, "");
        new FindCommand(user, "FIND happynews.users Article", server).process();

        assertThat(out.get(), equalTo("232 List of matching articles follows\n<123@host>\n.\n"));
    }
    ...
    public void doesntPrintUnmatchingArticles() throws IOException {
        ...
        User user = mockReader("reader");
        Supplier<String> out = mockInput(user, "");
        new FindCommand(user, "FIND happynews.users xpto", server).process();
    ...
    public void ignoresNonexistentGroups() throws IOException {
        ...
        User user = mockReader("reader");
        Supplier<String> out = mockInput(user, "");
        new FindCommand(user, "FIND happynews.users,nonexistent Article", server).process();
